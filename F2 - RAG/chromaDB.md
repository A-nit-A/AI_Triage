...
2. ¿En qué se diferencia de una base de datos SQL?
...
La diferencia principal radica en cómo almacenan la información y cómo la buscan.
    ...
    ChromaDB (Base de Datos Vectorial):
        ...
        Estructura: Guarda listas de números (vectores) junto con metadatos (y aquí es donde entra SQLite, como vimos antes, para organizar esos metadatos).
...
3. ¿Por qué se usa para RAG (Retrieval-Augmented Generation)?

RAG (Generación Aumentada por Recuperación) es una técnica que permite a un modelo de lenguaje (LLM, como ChatGPT o Gemini) responder preguntas basándose en tus propios documentos privados o en información actualizada, en lugar de usar solo lo que aprendió durante su entrenamiento.

ChromaDB es el "motor de memoria" perfecto para RAG por la siguiente secuencia de pasos:

    Ingesta: Tomas tus documentos (PDFs, manuales, bases de conocimiento), los cortas en pequeños párrafos y los conviertes en vectores usando un modelo de IA.

    Almacenamiento: Guardas todos esos vectores (y el texto original) en ChromaDB.

    Pregunta del usuario: Cuando un usuario hace una pregunta, esa pregunta también se convierte en un vector.

    Recuperación (Retrieval - El trabajo de ChromaDB): ChromaDB busca en su espacio matemático cuáles son los párrafos (vectores) más cercanos o relevantes a la pregunta del usuario.

    Generación (Generation - El trabajo del LLM): Se le envía al LLM la pregunta del usuario junto con los párrafos relevantes que encontró ChromaDB. El LLM lee esa información y genera una respuesta precisa y con contexto.

En resumen: Se usa ChromaDB para RAG porque los LLMs no pueden leer una biblioteca entera de documentos cada vez que les haces una pregunta (sería lentísimo y muy costoso). ChromaDB actúa como un bibliotecario ultrarrápido que, basándose en el "significado" de tu pregunta, va a la estantería, saca solo las 3 o 4 páginas que contienen la respuesta correcta, y se las entrega al LLM para que te explique la solución.

Es una excelente pregunta técnica. Para entender por qué ChromaDB usa SQLite, primero hay que aclarar un detalle arquitectónico clave: ChromaDB no usa SQLite para buscar los vectores (embeddings). La búsqueda de similitud vectorial la maneja un motor especializado (típicamente hnswlib). Entonces, ¿para qué usa SQLite? Lo utiliza exclusivamente para almacenar y consultar los metadatos, los IDs y los documentos asociados a esos vectores.

Aquí te explico las razones principales por las que los creadores de ChromaDB eligieron SQLite para esta tarea:
1. Fricción cero (Developer Experience)
...
2. Arquitectura Embebida (Embedded)
...
3. Filtrado eficiente de Metadatos
...
4. Persistencia simple basada en archivos
...
En resumen:
ChromaDB usa una arquitectura híbrida inteligente. Usa algoritmos en C++ (hnswlib) para la matemática pesada de los vectores, y usa SQLite porque es el estándar de oro para guardar datos estructurados localmente sin complicarle la vida al desarrollador.